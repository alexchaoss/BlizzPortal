...
class PlayerViewHolder(inflater: LayoutInflater, parent: ViewGroup, private val context: Context) :
    ...
    fun bind(player: Player, region: String) {
        var playerName = if(player.data.find { it.id == "HeroBattleTag" }?.string == null) {
            "unavailable"
        } else {
        ...
        //playerName += if(player.data.find { it.id == "HeroClanTag" }?.string == null) "" else  "[${player.data.find { it.id == "HeroClanTag" }?.string!!}]"
